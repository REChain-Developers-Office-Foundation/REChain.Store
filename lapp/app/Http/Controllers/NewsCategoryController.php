...
class NewsCategoryController extends Controller
{
    public function __construct()
    {
        ...
        // List of categories
        $rows = Category::where('type', '2')->orderBy('sort', 'ASC')->paginate(15);
    ...
    public function index(Request $request)
    {
        // Sort apps
        if ($request->has('sort')) {
            $posts = Category::where('type', '2')->orderBy('sort', 'ASC')->get();
        ...
        // Return view
        return view('adminlte::news.categories.index');
    }
    ...
    {
        // List of pages
        $rows = Category::where('type', '2')->orderBy('sort', 'ASC')->get();

        // Return view
        return view('adminlte::news.categories.sort')->with('rows', $rows);
    }
    ...
    {
        // Return view
        return view('adminlte::news.categories.create');
    }
    ...
    public function store(Request $request)
    {
        ...
        $slug_check = Category::where('type', '2')->where('slug', $request->get('slug'))->first();
        ...
        $category->sort = Category::where('type', '2')->max('sort') + 1;
        ...
        // Redirect back
        return redirect()->route('news-categories.edit', $category->id)->with('success', __('admin.content_added'));
    }
    ...
    public function edit(Request $request, $id)
    {
        ...
        // Retrieve details
        $row = Category::where('type', '2')->find($id);
        ...
        // Return view
        return view('adminlte::news.categories.edit', compact('row', 'id', 'languages', 'title'));
    }
    ...
    {
        // Check if slug exists
        $slug_check = Category::where('type', '2')->where('slug', $request->get('slug'))->where('id', '!=', $id)->first();
        ...
        // Redirect back
        return redirect()->route('news-categories.edit', $category->id)->with('success', __('admin.content_updated'));
    }
