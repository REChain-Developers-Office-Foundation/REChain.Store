    ...
    <div class="row">
        ...
        <div class="col-md-9 pe-md-2">
            ...
            <div class="shadow-sm p-2 app-details bg-white rounded mb-3">
                ...
                <div class="show-qr-code float-end">{!! svg_icon('fas fa-qrcode') !!}
                    <div class="qr-code bg-white p-1">
                        <img src="{{ asset(DNS2D::getBarcodePNGPath(url()->current(), 'QRCODE', 4.25, 4.25, '255,255,255', $app_query->slug)) }}" alt="@lang('general.qr_code')">
                    </div>
                </div>
                ...
                <div class="m-1" id="app" data-thumb="{{ s3_switch($app_query->image ?? 'no_image.png') }}" data-url="{{url()->current()}}" data-title="{{ $app_query->title ?: $app_query->main_title }}" data-site-title="{{ $settings['site_title'] }}" data-add-message="@lang('general.add_message')" data-remove-message="@lang('general.remove_message')" data-cookie-prefix="{{ $settings['cookie_prefix'] }}">

                    <div class="d-flex">
                        <div class="flex-shrink-0 me-1">
                            <img src="{{ s3_switch($app_query->image) }}" class="app-image" alt="{{ $app_query->title ?: $app_query->main_title }}">
                        </div>
                        <div class="flex-grow-1 ms-2 @if (!isset($app_query->url))my-auto @endif">
                            <h1>{{$h1_title}}</h1>
                            @if (isset($app_query->version))<span class="version">{{$app_query->version}}</span>@endif @if (isset($app_query->developer))<span class="developer my-1">@lang('general.by', ['developer' => $app_query->developer]) </span>@endif
                            <div class="clearfix"></div>
                            {!!stars($app_query->votes, 1)!!}@if($app_query->votes != '0.00')<span class="votes">/5</span>@endif
                            <span class="review-data ms-1">{!! svg_icon('fas fa-user') !!} ({{$app_query->total_votes}} @lang('general.reviews'))</span>
                    ...
                    <div class="smi row mx-0 mt-2 pt-1 text-center">
                        ...
                        <div class="col fav p-2 twitter"><a onclick="sm_share('http://twitter.com/share?text={{ $app_query->title ?: $app_query->main_title }}&url={{url()->current()}}','Twitter','600','300');" href="javascript:void(0);">{!! svg_icon('fab fa-twitter') !!} <span class="d-none d-lg-inline-block">Twitter</span></a></div>
                        ...
                        <div class="col fav p-2 email"><a href="mailto:?subject={{ $app_query->title ?: $app_query->main_title }}&amp;body={{url()->current()}}">{!! svg_icon('fas fa-envelope') !!} <span class="d-none d-lg-inline-block">E-mail</span></a></div>
                        <div class="col fav p-2 whatsapp"><a onclick="sm_share('https://api.whatsapp.com/send?text={{ $app_query->title ?: $app_query->main_title }} {{url()->current()}}','WhatsApp','700','650');" href="javascript:void(0);">{!! svg_icon('fab fa-whatsapp') !!} <span class="d-none d-lg-inline-block">WhatsApp</span></a></div>
                ...
                <div class="container screenshots">
                    <div class="row">
                        ...
                        <div id="screenshot-main">
                            ...
                            <a href="{{ s3_switch($image_name, 6) }}"><img src="{{ s3_switch('pixel.png') }}" data-src="{{ s3_switch($image_name, 6) }}" class="lazy mr-1" alt="{{ $app_query->title ?: $app_query->main_title }}"></a>
                            @endforeach
            ...
            <div class="shadow-sm p-2 app-details bg-white rounded mb-3">
                <div class="m-1">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4 col-6 mb-3"><b>@lang('general.report')</b><br><a href="return%20false;" class="report" data-bs-toggle="modal" data-bs-target="#MyModal">@lang('general.report_a_problem')</a></div>
                    </div>
            ...
            <div class="shadow-sm p-2 app-details bg-white rounded mb-3">
                <div class="m-1">
                    <h2 class="mb-3">@lang('general.old_versions_of') {{ $app_query->title ?: $app_query->main_title }}</h2>
                    @foreach ($versions as $version)

                    @if(!$loop->first)
                    <div class="d-flex mb-2">
                        <div class="flex-grow-1 my-auto">
                            <h4>{{ $app_query->title ?: $app_query->main_title }} {{$version->version}}</h4><span class="last-update">{{\Carbon\Carbon::parse($version->updated_at)->translatedFormat('F d, Y')}}</span> @if (isset($version->file_size))<span class="file-size ms-2">{{$version->file_size}}</span>@endif
                        </div>
            ...
            <div class="row mx-0 up-down mb-3">
                <div class="col-6 down shadow-sm rounded-start" id="vote-data" data-vote-success="@lang('general.vote_success')" data-vote-error="@lang('general.vote_error')">
            ...
            <div class="shadow-sm p-2 bg-white rounded pb-1">
                <div class="m-1">
                    ...
                    <a href="#" class="btn add-comment text-white float-end">@lang('general.add_comment_review')</a>
                    ...
                    <div class="user-reviews">
                        ...
                        <div class="comment-box" id="comment-section" data-fill-all-fields="@lang('general.fill_all_fields')">
                            ...
                            <form id="comment-form">
                                ...
                                <div class="mb-3">
                                    <label>@lang('general.your_rating'): <span class="text-danger">*</span></label>
                        ...
                        <div class="col-4 col-md-4 mb-md-3 mb-2">
                            <a href="{{ asset($language_prefix.$settings['app_base']) }}/{{ $app->slug }}">
                                <div class="row">
                                    ...
                                    <div class="col-md-8 ps-md-0 pt-md-0 pt-2"><span class="title">{{ $app->title ?? $app->main_title }}</span><span class="developer my-md-1 my-0">{{ $app->developer }}</span></div>
                                </div>
                            </a>
                        </div>
                        @endforeach
        ...
        <div class="modal-content">
            ...
            <div class="modal-body submission-box pt-1 pb-1" id="report-submission-section" data-station-id="{{ $app_query->id }}" data-error="@lang('general.error')" data-recaptcha-error="@lang('general.recaptcha_error')">
                ...
                <form id="report-submission-form">
                    ...
                    <div class="mt-3">
                        <input type="email" class="form-control" id="report_email" name="email" placeholder="@lang('general.email')" required>
                    </div>
