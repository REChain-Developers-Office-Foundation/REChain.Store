...
<div class="row">
    <div class="col-12">
        <a href="{{ asset(env('ADMIN_URL').'/topics/create') }}" class="btn button-green mb-3"><i class="fas fa-plus-square"></i>
            @lang('admin.create_topic')</a>
        <div class="card">
            <div class="table-responsive">
                <table class="table table-hover text-nowrap m-0" id="table" data-delete-prompt-title="@lang('admin.oops')" data-delete-prompt-body="@lang('admin.delete_prompt')" data-yes="@lang('admin.yes')" data-cancel="@lang('admin.cancel')">
                    ...
                    <tbody>
                        @foreach ($rows as $row)
                        <tr id="{{$row->id}}">
                            <td><img src="{{ s3_switch($row->image, 2) }}" class="img-w100"></td>
                            <td>{{$row->id}}</td>
                            <td><a href="{{ asset($settings['topic_base']) }}/{{ $row->slug }}" class="text-dark" target="_blank">{{$row->title}}</a></td>
                            ...
                            <td>
                                <div class="btn-group">
                                    ...
                                    <div class="dropdown-menu mr-3">
                                        ...
                                        <form id="delete_from_{{$row->id}}" method="POST" action="{{action('App\Http\Controllers\TopicController@destroy', $row['id'])}}">
                                            {{ csrf_field() }}
                                            {{ method_field('DELETE') }}
